...
| **el** | **data** | **watch** |
|---|---|---|
| Vue가 실행될 HTML의 DOM 요소를 지정(CSS의 선택자를 선택하듯이 선택 (#--> id 지정, . --> 클래스 지정)| Vue가 바라보는 data 객체를 지정(직접 객체를 작성해도 되고 미리 선언된 객체변수를 작성해도 됨)|지정된 변수를 계속 지켜보고 있다가 값이 변경되었을때 정의된 함수를 실행시킴(관찰하고자 하는 지정된 변수, 긴 시간이 필요한 비동기 처리가 필요할 경우 주로 사용)|알너이ㅏ렁라
